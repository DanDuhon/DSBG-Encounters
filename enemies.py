...
class Enemy:
    def __init__(self, name, expansion, enemyType, numberOfModels, health, armor, resist, iconForEffects, attacks, attackType, dodge, move, attackRange, id=None, repeat=0, nodeAttack=[], nodesAttacked=[], attackEffect=[], weakArcs=None, windup=False, moveIndex=None, skipDefense=False, difficultyTiers=0, moveAttack=[], modified=False, comboSet=frozenset(), aoe=False) -> None:
        ...
        attackEffect = [[attackEffect[i] for i in iconForEffects] for _ in attacks]
        ...
        if not modified:
            for combo in aoeNgpc if aoe else ngpc:
                ...
                if (
                    ...
                    or all([len(ae) + (1 if "effect1" in comboSet else 2 if "effect2" in comboSet else 0) > 2 for i, ae in enumerate(attackEffect) if attacks[i]])
                ...
                if {"effect1", "effect2"} & comboSet:
                    ...
                    for effectCombo in combinations_with_replacement(effectMods if "effect1" in comboSet else effectCombos, attacksForEffects):
                        modAttackEffects.append([effectCombo] if "effect1" in comboSet else list(effectCombo))
...
# Enemy(name="Voracious Mimic - Stomping Kick", moveAttack=[True], expansion="Explorers", enemyType="invader", numberOfModels=1, health=18, armor=2, resist=2, iconForEffects=[0,1], attacks=[7,7], attackType=["physical","physical"], nodeAttack=[True,True], dodge=1, move=[0,1], attackRange=[0,0])
