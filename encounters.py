...
skip = True
for e in enc:
    if e not in {"Fortress Gates","Blazing Furnace","Sewers of Lordran","Brume Tower","Courtyard of Lothric"}:
        continue
    #     skip = False
    # if skip:
    #     continue
    ...
    for characterCount in range(1, 5):
        ...
        # Then DSBG-Shuffle will stitch them together when needed.
            ...
            for tile in encounter["tiles"]:
                ...
                allCombos = list(set(combinations([enemyIds[en].id for en in allEnemies if (
                    (enemyIds[en].health in {
                        (en not in invaders or invaderCount > 0)
                        and enemyIds[enemies[0]].health,
                ...
                comboSet = [tuple(combo) for combo in allCombos if (
                            difficulty * (1 - diffMod) <= sum([enemyIds[enemy].difficulty[characterCount] * (1.5 if enemy in skeletons and blackHollowMage in enemies else 1) for enemy in combo]) <= difficulty * (1 + diffMod)
            ...
            [combosDict[frozenset([enemyIds[enemyId].expansion for enemyId in combo]).union({"Iron Keep"} if e in crystalLizardEncounters else set())].add(combo) for combo in allCombos if (
                ...
                and (e != "Trecherous Tower" or (combo.count(sorted([enemy for enemy in combo], key=lambda x: enemyIds[x].difficulty[characterCount], reverse=True)[0]) == 1 and combo.count(sorted([enemy for enemy in combo], key=lambda x: enemyIds[x].difficulty[characterCount], reverse=True)[1]) == 1))
                # One of the strongest enemy
                and (e != "Cold Snap" or combo.count(sorted([enemy for enemy in combo], key=lambda x: enemyIds[x].difficulty[characterCount], reverse=True)[0]) == 1)
                # A pair of enemies
                ...
                and (e != "Gleaming Silver" or (len(set([enemy for enemy in combo if combo.count(enemy) == 2])) == 2 and combo.count(sorted([enemy for enemy in combo], key=lambda x: enemyIds[x].difficulty[characterCount], reverse=True)[0]) == 1))
                # Two of the weakest 1 health enemy
                and (e not in set(["Skeletal Spokes", "The Shine of Gold"]) or ([enemy for enemy in combo if enemyIds[enemy].health == 1] and combo.count(sorted([enemy for enemy in combo if enemyIds[enemy].health == 1], key=lambda x: enemyIds[x].difficulty[characterCount])[0]) == 2))
                # Three of the weakest 1 health enemy, also no poison causing enemies
                and (e != "Shattered Keep" or (
                    combo.count(sorted([enemy for enemy in combo if enemyIds[enemy].health == 1], key=lambda x: enemyIds[x].difficulty[characterCount])[0]) == 3
                ...
                and (e != "Eye of the Storm" or (
                    (combo.count(sorted([enemy for enemy in combo], key=lambda x: enemyIds[x].difficulty[characterCount])[0]) == 4
                        or (combo.count(sorted([enemy for enemy in combo], key=lambda x: enemyIds[x].difficulty[characterCount])[0]) == 2
                            and combo.count(sorted([enemy for enemy in combo], key=lambda x: enemyIds[x].difficulty[characterCount])[2]) == 2))
                    and combo.count(sorted([enemy for enemy in combo], key=lambda x: enemyIds[x].difficulty[characterCount], reverse=True)[0]) == 1))
            )]
        ...
        # going to take a sample instead.
            ...
            for x in range(1, 3):
                for expCombo in combinations(expansions, x):
                    ...
                    [combosDict[frozenset([enemyIds[enemyId].expansion for enemyId in combo]).union({"Iron Keep"} if e in crystalLizardEncounters else set())].add(combo) for combo in allCombos if (
                        ...
                        and (e != "Frozen Revolutions" or combo.count(sorted([enemy for enemy in combo if enemyIds[enemy].health == 1], key=lambda x: enemyIds[x].difficulty[characterCount], reverse=True)[0]) == 2)
                        # If there are 2 of the strongest enemy and that enemy isn't in the blacklist
                        and (e != "Deathly Freeze" or (
                            combo.count(sorted(combo, key=lambda x: enemyIds[x].difficulty[characterCount], reverse=True)[0]) == 2
                            and sorted(combo, key=lambda x: enemyIds[x].difficulty[characterCount], reverse=True)[0] not in deathlyFreezeEnemyBlacklist))
                        # Two of the second strongest enemy (second strongest cannot also be the first strongest)
                        and (e != "Trophy Room" or (combo.count(sorted([enemy for enemy in combo], key=lambda x: enemyIds[x].difficulty[characterCount], reverse=True)[1]) == 2
                                                    and sorted([enemy for enemy in combo], key=lambda x: enemyIds[x].difficulty[characterCount], reverse=True)[1] == sorted(list(set([enemy for enemy in combo])), key=lambda x: enemyIds[x].difficulty[characterCount], reverse=True)[1]))
            ...
            while sum((len(combosDict[sets]) for sets in combosDict)) < 500000:
                ...
                allCombos = list(islice(combinations([en for en in shuffledEnemies if en not in invaders or invaderCount > 0], enemyCount), 50000))
